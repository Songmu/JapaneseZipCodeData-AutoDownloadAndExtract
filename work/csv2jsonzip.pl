...
# ---------------------------------------------------------------- #
    use strict;
    use utf8;
    use Encode qw/ encode decode /;                     # 文字コード変換モジュール弾
    use FindBin;
    use lib "$FindBin::Bin/lib";
    use JSON;                       # JSON.pm がデフォルトです
#   use JSON::Syck;                 # JSON::Syckがあれば利用可能です
    binmode STDOUT => ":encoding(cp932)"; # for Win32 command prompt
# ---------------------------------------------------------------- #
    my $VERSION     = '2.11';
    my $CSV_ENC     = 'CP932';        # CSVファイルのエンコーディング
    my $JSON_ENC    = 'utf8';         # JSONファイルのエンコーディング
    my $DISP_ENC    = 'utf8';         # 表示用のエンコーディング
    my $CSV_FILE    = $FindBin::Bin . '/ken_all.csv';  # 入力元CSVファイル名（デフォルト）
    my $CSV_JIGYO   = $FindBin::Bin . '/jigyosyo.csv'; # 事業所要郵便番号CSVファイル（オプション）
    my $JSON_BASE   = $FindBin::Bin . '/../data/zip-%s.json';  # 出力先JSONファイル名
# ---------------------------------------------------------------- #
    # local $| = 1;
    $| = 1;
    &main( @ARGV );
...
sub read_ken_all {
    ...
    print STDOUT decode( utf8 => "ken_all:\t$csvfile\n");
    ...
    while ( my $iline = <CSV> ) {
        ...
        if ( $prev ne $pref ) {
            $prev = $pref;
            print STDOUT decode( utf8 => " $c lines\n") if $c;
            my $v = sprintf( "%s  \t", $r[6] );
            $v = Encode::encode( $DISP_ENC, $v );
            print STDOUT decode( utf8 => $v);
            $c = 0;
        }
        print STDOUT decode( utf8 => ".") if ( $c ++ % 200 == 0 );
    }
    print STDOUT decode( utf8 => " $c lines\n") if ( $c > 0 );
    close( CSV );
    $out;
...
sub read_jigyosyo {
    ...
    print STDOUT decode( utf8 => "jigyosyo:\t$csvjigyo\n");
...
sub write_json {
    ...
    print STDOUT decode( utf8 => "module: \tJSON.pm ($use_json)\n") if $use_json;
    print STDOUT decode( utf8 => "module: \tJSON::Syck ($use_syck)\n") if $use_syck;
    print STDOUT decode( utf8 => "json:   \t$JSON_BASE\n");
    ...
    foreach my $zip3 ( sort keys %$out ) {
        ...
        if ( $prev ne $zip1 ) {
            $prev = $zip1;
            print STDOUT decode( utf8 => " $c files\n") if $c;
            printf STDOUT ( decode( utf8 => "$JSON_BASE  "), decode( utf8 => "$zip1**" ));
            $c = 0;
        }
        print STDOUT decode( utf8 => ".") if ( $c ++ % 10 == 0 );
    }
    print STDOUT decode( utf8 => " $c files\n") if ( $c > 1 );
}
# ---------------------------------------------------------------- #
